...
import { CredentialSecretToast } from './credential-secret-toast';
...
const CredentialCreateButton = ({
  ...
  const handleSuccess = (successActionState: ActionState) => {
    handleClose();

    const secret = successActionState.data?.secret;

    if (!secret) {
      return;
    }

    toast.custom((id) => <CredentialSecretToast id={id} secret={secret} />, {
      duration: Infinity,
    });
  };
